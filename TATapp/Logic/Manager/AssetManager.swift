...
class AssetManager {
    ...
    class func deleteLocalTempFile(filename: String){
        if AssetManager.fileExistsAt(filename: filename) {
            let localFile = AssetManager.getFileNameWithRootDocument(filename: filename)
            AssetManager.deleteFile(url: localFile)
        }
    }
